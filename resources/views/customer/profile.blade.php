...
@section('content')
    ...
    <div style="display: grid; grid-template-columns: 320px 1fr; gap: 20px; align-items: start;">
        ...
        <div>
            {{-- Hero Card --}}
            <div
                style="background: linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%); border-radius: 14px; padding: 28px 24px; margin-bottom: 16px; text-align: center; position: relative; overflow: hidden;">
                <div
                    style="position:absolute;inset:0;background:url('data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'%23ffffff\' fill-opacity=\'0.03\'%3E%3Cpath d=\'M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z\'/%3E%3C/g%3E%3C/svg%3E');">
                </div>
                <div style="position:relative;">
                    <div
                        style="width:72px; height:72px; border-radius:50%; background:linear-gradient(135deg, #1e40af, #3b82f6); display:flex; align-items:center; justify-content:center; font-size:24px; font-weight:700; color:#fff; margin:0 auto 14px; border:3px solid rgba(255,255,255,0.15);">
                        {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                    </div>
                    <div style="font-size:17px; font-weight:700; color:#fff;">{{ Auth::user()->name }}</div>
                    <div style="font-size:12px; color:rgba(255,255,255,0.45); margin-top:3px;">{{ Auth::user()->email }}
                    </div>
                    <div
                        style="margin-top:12px; display:inline-block; background:rgba(30,64,175,0.4); border:1px solid rgba(59,130,246,0.3); border-radius:6px; padding:3px 12px; font-size:11px; font-weight:600; color:#93c5fd;">
        ...
        <div style="display: flex; flex-direction: column; gap: 16px;">
            ...
            <div class="card">
                <div
                    style="padding: 18px 20px 14px; display:flex; align-items:center; justify-content:space-between; border-bottom:1px solid #f1f5f9;">
                    ...
                    <button onclick="openModal('profileModal')"
                        style="font-size:12px; font-weight:600; color:#1e40af; background:none; border:none; cursor:pointer; display:flex; align-items:center; gap:4px;">
                ...
                <div style="padding: 8px 0;">
                    @php
                        $infoRows = [
                            ...
                            ['label' => 'Phone', 'value' => Auth::user()->phone, 'icon' => 'M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.67A2 2 0 012 .14h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 14l.01 2.92z'],
                    ...
                    @foreach($infoRows as $row)
                        <div style="display:flex; align-items:center; padding:12px 20px; border-bottom:1px solid #f8fafc;">
                            <div
                                style="width:32px; height:32px; background:#f8fafc; border-radius:8px; display:flex; align-items:center; justify-content:center; margin-right:12px; flex-shrink:0;">
            ...
            <div class="card">
                ...
                <div style="padding: 16px 20px;">
                    <div style="font-size:13px; color:#334155; line-height:1.7;">
                        {{ Auth::user()->delivery_address ?? 'No address found' }}
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div
                    style="padding: 18px 20px 14px; display:flex; align-items:center; gap:10px; border-bottom:1px solid #f1f5f9;">
                    <div
                        style="width:32px;height:32px;background:#fff7ed;border-radius:8px;display:flex;align-items:center;justify-content:center;">
                ...
                <div style="padding: 14px 20px; display:flex; align-items:center; justify-content:space-between;">
                    ...
                    <button onclick="openModal('passwordModal')"
                        style="background:#eff6ff; border:none; border-radius:8px; padding:8px 14px; font-size:12px; font-weight:600; color:#1e40af; cursor:pointer;">
                        Change
                    </button>
                </div>
            </div>
    ...
    <div id="profileModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <div class="modal-header">
                Edit Profile
                <span onclick="closeModal('profileModal')" style="cursor:pointer;">✖</span>
            </div>
    ...
    <div id="addressModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <div class="modal-header">
                Delivery Address
                <span onclick="closeModal('addressModal')">✖</span>
            </div>
            ...
            <form method="POST" action="/profile/address">
                ...
                <div class="modal-body">
                    <textarea name="delivery_address" rows="4">{{ Auth::user()->delivery_address }}</textarea>
                </div>
    ...
    <div id="passwordModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <div class="modal-header">
                Change Password
                <span onclick="closeModal('passwordModal')">✖</span>
            </div>
    ...
    <script>
        function openModal(id) {
            document.getElementById(id).style.display = 'flex'; // ✅ FIX
        }
